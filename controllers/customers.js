...
const getSingle = (req, res) => {
    ...
    if (!ObjectId.isValid(req.params.id)) {
        res.status(500).json('Must use a valid customer id to find a customer');
    }
...
const updateCustomer = async (req, res) => {
    //#swagger.tags=['customers']
    if (!ObjectId.isValid(req.params.id)) {
        res.status(500).json('Must use a valid customer id to update a customer');
    }
    const customerId = new ObjectId(req.params.id);
    const customer = {
        firstname:req.body.firstname,
...
const deleteCustomer = async (req, res) => {
    //#swagger.tags=['customers']
    if (!ObjectId.isValid(req.params.id)) {
        res.status(500).json('Must use a valid customer id to delete a customer');
    }
    const customerId = new ObjectId(req.params.id);
